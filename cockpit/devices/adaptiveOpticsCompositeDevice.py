## Collection of methods which use the Adaptive Optics Composite device
# !/usr/bin/python
# -*- coding: utf-8
#
# Copyright 2018 Nick Hall (nicholas.hall@dtc.ox.ac.uk)
...
class MicroscopeCompositeAO(MicroscopeBase, device.Device):
    def __init__(self, name, compDM_config={}):
        super(self.__class__, self).__init__(name, compDM_config)
        self.proxy = None
        self.sendImage = False
        self.curCamera = None

        self.buttonName = 'Adaptive Optics Composite Device'
    ...
    def correctSensorlessProcessing(self):
        ...
        if len(self.correction_stack) < self.zernike_applied.shape[0]:
            ...
            log_file.write("Time stamp: %i:%i:%i %i/%i/%i\n" % (
                time.gmtime()[3], time.gmtime()[4], time.gmtime()[5], time.gmtime()[2], time.gmtime()[1],
                time.gmtime()[0]))
...
class dmOutputWindow(wx.Frame):
    def __init__(self, CompAoDevice, parent, *args, **kwargs):
        wx.Frame.__init__(self, parent, *args, **kwargs)
        ## Composite AO Device instance.
        self.dm = CompAoDevice
